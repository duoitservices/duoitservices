...
export default function NewTicket() {
  ...
  const { data: allTickets = [] } = useQuery({
    queryKey: ['allTickets'],
    queryFn: async () => {
      return base44.entities.Ticket.list('-created_date');
    }
  });

  // Filtrar tickets pelo contrato selecionado
  const relatedTickets = formData.contract_id
    ? allTickets.filter(t => t.contract_id === formData.contract_id)
    : [];
  ...
  // Use currentUser as systemUser
  const systemUser = currentUser;

  // Helper: contract is operational (status Ativo AND end_date not passed)
  ...
  // Filtrar apenas contratos ativos e dentro do prazo, nos quais o usuário está vinculado
  const userContracts = allContracts.filter(c => {
    if (!isContractOperational(c)) return false;
  ...
  console.log('🔍 [NewTicket] Debug Info:', {
    currentUserEmail: currentUser?.email,
    systemUserEmail: systemUser?.email,
    allContracts: allContracts.length,
    userContracts: userContracts.length,
    contractsWithUsers: allContracts.filter(c => c.linked_users?.length > 0).length
  });

  // Extrair parceiros únicos dos contratos do usuário
  ...
  // Filtrar contratos por parceiro selecionado
  ...
  const createTicket = useMutation({
    mutationFn: async (data) => {
      ...
      // Find matching SLAs - prioriza SLAs do contrato específico, com fallback para parceiro
      const findSLA = (type) => {
        // 1. Busca por contrato + tipo de chamado
        let found = slas.find(s =>
          s.sla_type === type &&
          s.contract_id === data.contract_id &&
          s.ticket_type === data.ticket_type &&
          s.active !== false
        );
        // 2. Busca por contrato sem tipo de chamado
        if (!found) found = slas.find(s =>
          s.sla_type === type &&
          s.contract_id === data.contract_id &&
          !s.ticket_type &&
          s.active !== false
        );
        // 3. Fallback: busca por parceiro + tipo de chamado
        if (!found) found = slas.find(s =>
          s.sla_type === type &&
        ...
        // 4. Fallback: busca por parceiro sem tipo de chamado
        if (!found) found = slas.find(s =>
          s.sla_type === type &&
          s.partner_id === data.partner_id &&
          !s.ticket_type &&
      ...
      const responseSLA = findSLA('Resposta');
      const solutionSLA = findSLA('Solução');
      ...
      // Calculate deadlines considering calendar
      const calculateDeadline = (hours, calendarId) => {
        ...
        // Simple calculation - can be enhanced with actual business hours
        return new Date(Date.now() + hours * 60 * 60 * 1000).toISOString();
      };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Select 
                  ...
                  onValueChange={(v) => setFormData({ ...formData, related_ticket_id: v })}
                  disabled={!formData.contract_id}
                >
                  <SelectTrigger className="mt-1">
                    <SelectValue placeholder={formData.contract_id ? "Selecione um chamado" : "Selecione o contrato primeiro"} />
                  </SelectTrigger>
                  <SelectContent>
